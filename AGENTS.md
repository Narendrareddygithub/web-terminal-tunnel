...
- `terminal_server.py` — FastAPI app (only backend). Serves an embedded HTML page (`INDEX_HTML` string, xterm.js from CDN) at `/`, a JSON shell list at `/shells` (plus `POST /rescan` to re-probe agents), and a WebSocket at `/ws`. Runs as a **session hub**: `_sessions` registry holds one session per ConPTY (`pywinpty.PtyProcess`, whitelisted via `SHELLS`: powershell, pwsh, cmd, bash (Git Bash), wsl). `/ws` has two modes: no `session` query param = dashboard/control (state, create, close); `session=<id>` = terminal attach (input/resize/raw output). Owner default via `TERMINAL_SHELL`; `TERMINAL_SHELL_CHOICE` gates the shell dropdown when creating sessions.
...
Publishing to npm: `npm login` then `npm publish` (2FA/OTP or a granular bypass token required). Never commit `.npmrc` — gitignored. After a wrapper change: `npm pack` → `npm i -g ./wtt-web-1.2.0.tgz` → run + Ctrl+C teardown check before publishing.
...
- **AI agent harness detection + launch** (Claude Code, OpenCode, Codex, Aider, agy/Gemini, Goose, OpenHands, ...): DONE — `agents.py` registry, landing-page list, dashboard agent tabs + grouped dropdown, per-session cwd picker, rescan (see Layout/Gotchas).
